...
export default function TrustChangeCard({ userId }) {
  ...
  const stateLabel = score > 20 ? 'Stable' : score > 5 ? 'Building' : score > 0 ? 'Initial' : 'Starting';
  const stateColor = score > 20 ? '#22c55e' : score > 5 ? '#3b82f6' : score > 0 ? '#f59e0b' : '#94a3b8';

  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-3.5 flex items-center gap-3" data-testid="trust-change-card">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-gray-400">Field Trust</span>
          <ArrowUpRight className="w-3 h-3 text-green-500" />
        </div>
        <p className="text-[10px] text-gray-400">Status: <span className="font-medium" style={{ color: stateColor }}>{stateLabel}</span></p>
      </div>
