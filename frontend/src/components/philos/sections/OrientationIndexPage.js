...
export default function OrientationIndexPage() {
  ...
  return (
    <section className="bg-white rounded-3xl p-5 shadow-sm border border-border" dir="rtl" data-testid="orientation-index-page">
      {/* Header */}
        ...
        {directions.map(([key, meta]) => {
          ...
          return (
            <div key={key} className="flex flex-col items-center flex-1 h-full justify-end">
              <span className="text-xs font-bold mb-1" style={{ color: meta.color }}>{pct}%</span>
              <div className="w-full max-w-[48px] relative rounded-t-xl transition-all duration-700" style={{ height: `${Math.max(heightPct, 4)}%`, backgroundColor: meta.color, opacity: isDominant ? 1 : 0.6 }}>
                {isDominant && (
